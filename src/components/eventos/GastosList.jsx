...
const GastosList = ({ gastos = [], onAddGasto, onDeleteGasto, readOnly = false }) => {
  const [nuevoGasto, setNuevoGasto] = useState({ concepto: '', monto: '' });

  const handleAdd = () => {
    if (nuevoGasto.concepto.trim() && nuevoGasto.monto && onAddGasto) {
      onAddGasto({
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-4">
      ...
      {/* Inputs para agregar - SOLO visibles si NO es readOnly */}
      {!readOnly && (
